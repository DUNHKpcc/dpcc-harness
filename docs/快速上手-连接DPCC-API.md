...
打开后台 <https://api.dpccgaming.xyz> 并登录，准备好以下信息：

| 凭证 | 用途 | 是否必填 |
| --- | --- | --- |
| **网关地址** | 默认 `https://api.dpccgaming.xyz`，一般保持默认即可 | 必填 |
| **Claude 密钥（sk-…）** | Claude 会话、模型列表、余额查询 | Claude 用户必填 |
| **Codex 密钥（sk-…）** | Codex 会话、模型列表 | 仅用 Codex 时填 |
| **用户 ID + 访问令牌** | 显示账户**真实余额**（可选增强项） | 选填 |

> 令牌在后台「令牌 / API Key」页面新建并复制；用户 ID 和系统访问令牌在「用户设置」页面获取。

充值入口：<https://dpccgaming.xyz/payment>
