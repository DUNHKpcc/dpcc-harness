...
PccAgent 不直接调用官方 Claude / Codex，而是通过 **DPCC API 网关**调用。首次启动时，通过系统浏览器登录并授权当前设备即可，无需手工复制 API 密钥。
...
5. 账户状态显示**已连接**，Claude 与 Codex 的可用模型会自动加载。重新授权期间，旧连接会保留到新凭据确认成功。

---

## 三、不登录并继续

点击**不登录并继续**可进入访客模式。访客模式不会使用之前保存的 DPCC 设备凭据，Claude/Codex 会改用本地或你另行配置的自定义网关。需要恢复默认 DPCC 连接时，再点击**使用浏览器登录**。
...
- **会话因授权失效而结束**：服务端拒绝或撤销设备凭据后，使用该 DPCC 凭据的 Claude/Codex 会话会立即停止；重新授权后再新建会话。
- **模型列表或余额无法刷新**：确认网络可访问 `origin-api.dpccgaming.xyz`，再重新授权。
- **凭证安全**：Claude/Codex 凭据只保存在 Electron `safeStorage` 支持的操作系统安全存储中；PccAgent 不提供再次查看或复制凭据的入口。
- **退出设备**：点击「退出并撤销此设备」会撤销服务端的 Claude 与 Codex 凭据并清理本机安全存储。
