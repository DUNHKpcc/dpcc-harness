...
export function AppLayout() {
  ...
  // Always-fresh manager handle for async flows that
  ...
  const mainToolWorkspace = useMainToolWorkspace(mainWorkspaceProjectId, manager.activeSessionId, {
  ...
  const createSplitPaneDraftSession = useCallback(
    async (replacedSessionId: string, projectId: string, agent: InstalledAgent | null) => {
      const selectedAgent = agent?.engine === "acp" ? agent : BUILTIN_PI_AGENT;
  ...
  const requestAddSplitSession = useCallback((sessionId: string, position?: number) => {
    const result = splitView.requestAddSplitSession({
      // Read the live active session id so split requests remain correct after
      // an asynchronous session switch.
      sessionId,
  ...
  const hasActiveRuntimeSession = !!manager.activeSessionId
    && !manager.isDraft
    && readOnlyReason === null;
  ...
  const renderMainWorkspaceToolContent = useCallback((
    ...
    <ToolIslandContent
      ...
      {...toolIslandCtx}
    />
  ), [activeProjectId, activeProjectPath, activeSpaceProject?.path, hasActiveRuntimeSession, manager.activeSession?.engine, manager.activeSessionId, manager.messages, spaceManager.activeSpaceId, toolIslandCtx]);
  ...
  return (
    ...
    >
            ...
            {jiraBoardProject ? (
              ...
              >
                <ChatHeader
                  ...
                  planMode={activePaneCtrl?.panePlanMode ?? false}
      ...
      {!welcomeCompleted && (
        <WelcomeWizard
          glassSupported={glassSupported}
          permissionBehavior={settings.acpPermissionBehavior}
          onPermissionBehaviorChange={settings.setAcpPermissionBehavior}
          onCreateProject={handleCreateProject}
          hasProjects={hasProjects}
