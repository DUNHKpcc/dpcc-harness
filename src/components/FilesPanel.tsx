...
export const FilesPanel = memo(function FilesPanel({
  ...
  const fileBrowserResize = useFileBrowserResize("open-files");
  ...
  return (
    <div
      data-file-browser-layout="open-files"
    ...
    >
      <div ref={fileBrowserResize.contentRef} className="file-browser-content min-h-0 flex-1" style={fileBrowserResize.contentStyle}>
        <section data-file-browser-preview className="file-browser-preview">
          <InlineFilePreview filePath={selectedFilePath} isListHidden={isListHidden} onToggleList={fileBrowserResize.toggleListVisibility} listToggleLabel={isListHidden ? t("files.showList") : t("files.hideList")} />
        </section>
        {!isListHidden && (
          <>
            <div
